...
test.describe('AdminPage', () => {
    ...
    test('should render AdminPage with all necessary elements', async ({ page }) => {
        ...
        await expect(page.locator('button:has-text("Create Exhibition")')).toBeVisible(); // Check the button
        await expect(page.locator('.ant-tabs-tab')).toHaveCount(3); // Ensure 3 tabs are present
    });

    test('should open Create Exhibition modal on button click', async ({ page }) => {
        await page.goto(adminPageURL);
        await page.click('button:has-text("Create Exhibition")');
        await expect(page.locator('.ant-modal-title')).toHaveText('Create Exhibition'); // Check modal title
    });

    test('should close the modal when cancel button is clicked', async ({ page }) => {
        await page.goto(adminPageURL);
        await page.click('button:has-text("Create Exhibition")');
    ...
    test('should switch between tabs correctly', async ({ page }) => {
        ...
        await expect(page.locator('.ant-tabs-tab-active')).toHaveText('Manage Users');

        // Switch to Manage Photos tab
        await page.click('.ant-tabs-tab:has-text("Manage Photos")');
        await expect(page.locator('.ant-tabs-tab-active')).toHaveText('Manage Photos');
    ...
    test('should render exhibitions in the Exhibition tab', async ({ page }) => {
        ...
        const exhibitions = ['Nature Photography', 'Urban Exploration', 'Artistic Vision'];
        for (const exhibition of exhibitions) {
            await expect(page.locator(`text=${exhibition}`)).toBeVisible();
        }
    });

    test('should render CreateExhibition component in the modal', async ({ page }) => {
        await page.goto(adminPageURL);
        await page.click('button:has-text("Create Exhibition")');
    ...
    test('should render correctly on different screen sizes', async ({ page }) => {
        ...
        for (const viewport of viewports) {
            ...
            await expect(page.locator('button:has-text("Create Exhibition")')).toBeVisible();
        }
    });
